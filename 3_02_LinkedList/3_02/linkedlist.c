...
LinkedList* createLinkedList() {            //포인터 함수?
        ...
        if (position >= 0 && position <= pList->currentElementCount) {
            ...
            if (pNewNode != NULL) {
                *pNewNode = element;            //필드가 추가되더라도 수정할 필요가 없도록 참조 연산자 사용
                ...
                pPreNode = &(pList->headerNode);        //헤더노드의 주소값을 PreNode에 저장하나 실제로 헤더노드가 되는 것임
                for(i=0; i<position; i++) {
                    pPreNode = pPreNode->pLink;         //PreNode가 가르키는 pLink가 결국 다음 노드를 가리킴
                }
...
void clearLinkedList(LinkedList* pList) {
    if (pList != NULL) {
        if (pList->currentElementCount >0) {
            removeLinkedListElement(pList, 0);      //첫번째만 삭제???
        }
    }
}
